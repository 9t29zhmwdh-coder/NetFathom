"""netfathom dashboard: optional web dashboard."""
...
def dashboard(host: str, port: int, open_browser: bool) -> None:
    ...
    Example:
      netfathom dashboard
      netfathom dashboard --port 9090 --no-open-browser
    """
...
async def _run(host: str, port: int, open_browser: bool) -> None:
    ...
    from netfathom.dashboard.server import app

    url = f"http://{'localhost' if host == '0.0.0.0' else host}:{port}"
    console.print(f"[bold green]NetFathom Dashboard[/bold green]  {url}")

"""netfathom speedtest: TCP/UDP throughput, latency, jitter."""
...
from netfathom.output import emit_json, emit_yaml, print_speedtest
...
def speedtest(
    ...
    \b
    Two-step usage (P2P):
      Host A (server):  netfathom speedtest --server
      Host B (client):  netfathom speedtest 192.168.1.10

    \b
    Or test latency only (no server needed):
      netfathom speedtest 8.8.8.8 --no-tcp --no-udp

    \b
    Examples:
      netfathom speedtest --server
      netfathom speedtest 192.168.1.10 --duration 30
      netfathom speedtest 1.1.1.1 --no-tcp --no-udp --pings 50
    """
    if server:
...
async def _run_server(port: int) -> None:
    from netfathom.performance.speedtest import SpeedtestServer
...
async def _run_client(
    ...
    from netfathom.performance.speedtest import SpeedtestClient

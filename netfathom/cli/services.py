"""netfathom services: mDNS, SSDP, NetBIOS, SNMP discovery."""
...
from netfathom.models import ServicesResult
from netfathom.output import emit_json, emit_yaml, print_services
...
def services(
    ...
    Examples:
      netfathom services
      netfathom services 192.168.1.0/24 --netbios --snmp
      netfathom services --no-mdns --ssdp --format json
    """
    asyncio.run(
        _run(
...
async def run_services_scan(
    ...
    from netfathom.discovery.mdns import MDNSDiscovery
    from netfathom.discovery.netbios import NetBIOSScanner
    from netfathom.discovery.snmp import SNMPScanner
    from netfathom.discovery.ssdp import SSDPScanner
...
async def _get_cache_ips() -> list[str]:
    from netfathom.scanner.layer2 import get_arp_cache

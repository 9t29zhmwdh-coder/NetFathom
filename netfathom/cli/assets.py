"""netfathom assets: list the persisted device inventory."""
...
from netfathom.output import emit_json, emit_yaml
...
async def _run(fmt: str, db_path: str | None) -> None:
    ...
    if fmt == "json":
        ...
        if not rows:
            console.print(
                "[yellow]No devices in inventory yet -- run `netfathom baseline` or `netfathom discover --persist` first.[/yellow]"
            )
            return
        table = Table(title=f"NetFathom Asset Inventory ({len(rows)} devices)", box=box.ROUNDED)

"""netfathom discover: host discovery via ARP, ICMP, TCP."""
...
from netfathom.models import DiscoverResult, Host
from netfathom.output import emit_json, emit_yaml, print_discover
from netfathom.scanner.hostname import resolve_hostnames_batch
from netfathom.scanner.layer2 import ARPScanner, get_arp_cache
from netfathom.scanner.layer3 import ICMPScanner
from netfathom.scanner.layer4 import SYNScanner, TCPScanner, parse_port_spec
from netfathom.scanner.privileges import is_root
...
def discover(
    ...
    Examples:
      netfathom discover
      netfathom discover 10.0.0.0/24 --arp --ports 22,80,443
      sudo netfathom discover --arp --syn --vendor --format json
    """
    asyncio.run(
        _run(
...
async def _run(
    ...
    if persist:
        ...
        if changes:
            console.print(
                f"[cyan]{len(changes)} change(s) detected[/cyan] -- run `netfathom changes` for details"
            )
        else:

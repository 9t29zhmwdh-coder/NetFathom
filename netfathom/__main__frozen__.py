"""PyInstaller entry point. Not used by `pip install netfathom` -- only
referenced by the build/*.spec files' Analysis(['netfathom/__main__frozen__.py']).

Double-clicking the frozen binary with no arguments launches the dashboard
(mirrors `netfathom dashboard`'s existing auto-open-browser behavior), since
...
def main() -> None:
    from netfathom.cli.main import cli

    if len(sys.argv) == 1:
        sys.argv.append("dashboard")
    cli(prog_name="NetFathom")

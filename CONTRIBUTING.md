...
2. `git clone https://github.com/YOUR_USERNAME/NetFathom`
3. `cd NetFathom`
...
3. Build the portable launcher locally for at least one OS and validate it manually: `pyinstaller build/netfathom-<os>.spec --workpath build/_work`, then run the resulting binary from an empty folder outside the dev environment (simulating the USB use case). Confirm `discover`, `services`, and `dashboard` all work, and that the dashboard's static assets load (no 404s). PyInstaller's dependency detection misses some dynamically-imported modules (see the `hiddenimports` comments in `build/*.spec`); a build that succeeds does not guarantee it runs.

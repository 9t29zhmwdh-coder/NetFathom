...
class LocalHealthRunner:
    ...
    async def _check_memory(self) -> HealthCheck:
        ...
        return HealthCheck(
            name="Memory Usage",
            status=status,
            message=f"{mem.percent:.0f}% RAM used",
            weight=15,
    ...
    async def _check_defender_status(self) -> HealthCheck:
        if sys.platform != "win32":
            return HealthCheck(
                ...
                status="skipped",
                message="Windows Defender only available on Windows",
                weight=20,
            )
        try:
    ...
    async def _check_bitlocker_status(self) -> HealthCheck:
        if sys.platform != "win32":
            return HealthCheck(
                ...
                status="skipped",
                message="BitLocker only available on Windows",
                weight=15,
            )
        try:
            output = await asyncio.to_thread(
    ...
    async def _check_windows_update(self) -> HealthCheck:
        if sys.platform != "win32":
            return HealthCheck(
                name="Windows Update",
                status="skipped",
                message="Windows Update only available on Windows",
                weight=15,
            )
        try:
            output = await asyncio.to_thread(
            ...
            if not last_update:
                return HealthCheck(
                    name="Windows Update",
                    status="skipped",
                    message="Could not determine last update date",
                    weight=15,
                )
            return HealthCheck(
                name="Windows Update",
